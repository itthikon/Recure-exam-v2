...
// Helper function to dynamically initialize Firebase for Firestore
function getFirebaseFirestore() {
  const projectId = appletConfig.projectId || process.env.FIREBASE_PROJECT_ID || 'exam-77ad9';
  const apiKey = appletConfig.apiKey || process.env.FIREBASE_API_KEY;
  const authDomain = appletConfig.authDomain || process.env.FIREBASE_AUTH_DOMAIN || `${projectId}.firebaseapp.com`;
  const storageBucket = appletConfig.storageBucket || process.env.FIREBASE_STORAGE_BUCKET || `${projectId}.firebasestorage.app`;
  const messagingSenderId = appletConfig.messagingSenderId || process.env.FIREBASE_MESSAGING_SENDER_ID;
  const appId = appletConfig.appId || process.env.FIREBASE_APP_ID;
  const databaseId = appletConfig.firestoreDatabaseId;

  // 1. Try Firebase Admin if service account credentials are provided
  ...
  if (clientEmail && privateKey) {
    try {
      ...
      if (adminAny.apps && adminAny.apps.length > 0) {
        adminApp = adminAny.app('firebase-admin-primary');
      } else {
        adminApp = adminAny.initializeApp({
          credential: adminAny.credential.cert({ projectId, clientEmail, privateKey }),
  ...
  // 2. Initialize using Firebase Client SDK with applet config
  try {
    const apps = getClientApps();
    const app = !apps.length ? initClientApp({
      apiKey,
      authDomain,
      projectId,
      storageBucket,
    ...
    const db = databaseId ? getClientFirestore(app, databaseId) : getClientFirestore(app);
    ...
    const adapter = {
      ...
      collection: (colName: string) => ({
        colName,
        get: async () => {
          const snap = await clientGetDocs(clientCollection(db, colName));
          return {
            empty: snap.empty,
            size: snap.size,
        ...
        limit: (num: number) => ({
          get: async () => {
            const snap = await clientGetDocs(clientQuery(clientCollection(db, colName), clientLimit(num)));
            return {
              empty: snap.empty,
              size: snap.size,
              docs: snap.docs.map(d => ({ id: d.id, data: () => d.data() }))
        ...
        doc: (docId?: string) => {
          const dRef = docId ? clientDoc(db, colName, String(docId)) : clientDoc(clientCollection(db, colName));
          return {
            id: dRef.id,
            colName,
            set: async (data: any, opts?: any) => clientSetDoc(dRef, data, opts || {}),
            delete: async () => clientDeleteDoc(dRef)
          };
        }
      }),
      batch: () => {
        ...
        return {
          set: (docRefWrapper: any, data: any, opts?: any) => {
            const col = docRefWrapper.colName || 'items';
            const dRef = clientDoc(db, col, String(docRefWrapper.id));
            b.set(dRef, data, opts || {});
          },
    ...
    return { firestore: adapter, useFirebase: true, projectId };
  } catch (err: any) {
    console.warn('Firebase initialization notice:', err.message);
    return { firestore: null, useFirebase: false, error: err.message, projectId };
  }
}
...
// Preload from offline_db.json if present
try {
  const offlinePath = path.join(rootDir, 'data/offline_db.json');
  if (fs.existsSync(offlinePath)) {
    const raw = fs.readFileSync(offlinePath, 'utf8');
    const parsed = JSON.parse(raw);
    defaultDbData = { ...defaultDbData, ...parsed };
  }
} catch (err) {
  console.warn('Notice loading local offline_db.json fallback:', err);
}
      ...
      for (const colName of collections) {
        ...
        for (let i = 0; i < items.length; i += chunkSize) {
          ...
          for (const item of chunk) {
            const docId = item.id || item.student_id || item.code || `${colName}_${Date.now()}`;
    ...
    if (studentLocal) {
      ...
      if (useSupabase) {
        supabase
          ...
          .catch((e: any) => console.error('Background sync failed:', e));
      }
